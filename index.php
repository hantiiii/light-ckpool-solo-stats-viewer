...
// --- API SECTION ---
if (isset($_GET['fetch_chart_data']) || isset($_GET['fetch_network_chart'])) {
    ...
    try {
        ...
        if (isset($_GET['fetch_network_chart'])) {
            ...
            $datasets['hashrate'] = ['labels' => [], 'data' => []];
            $datasets['difficulty'] = ['labels' => [], 'data' => []];
            foreach ($results as $row) {
                $ts = $row['timestamp'] * 1000;
                $datasets['hashrate']['labels'][] = $ts;
                $datasets['hashrate']['data'][] = round($row['network_hashrate_ghs'], 2);
                $datasets['difficulty']['labels'][] = $ts;
                $datasets['difficulty']['data'][] = round($row['network_difficulty'], 2);
            }
        } else {
...
if ($user_summary && $network_hashrate) {
    ...
    $analytics = [ 'prob_month' => calculate_block_probability($user_hashrate_ghs, $network_hashrate, 30.44), 'prob_year' => calculate_block_probability($user_hashrate_ghs, $network_hashrate, 365.25), 'time_to_find' => calculate_time_to_find_block($user_hashrate_ghs, $network_hashrate) ];
}
$pool_time_to_block = null;
if ($pool_data && $network_hashrate) {
    $pool_hashrate_str = $pool_data['hashrate1hr'] ?? '0';
    $pool_hashrate_ghs = parse_hashrate_to_ghs($pool_hashrate_str);
    $pool_time_to_block = calculate_time_to_find_block($pool_hashrate_ghs, $network_hashrate);
}
$estimated_adjustment_date = null;
if ($difficulty_prediction && isset($difficulty_prediction['progress'], $difficulty_prediction['avg_time']) && $difficulty_prediction['avg_time'] > 0) {
    $blocks_remaining = 2016 * (1 - ($difficulty_prediction['progress'] / 100));
    $seconds_remaining = $blocks_remaining * $difficulty_prediction['avg_time'];
    $estimated_timestamp = time() + $seconds_remaining;
    $estimated_adjustment_date = date('Y-m-d H:i', $estimated_timestamp);
}
$pool_title = "srv.88x.pl Mining Pool";
$pool_subtitle = "Bitcoin Mining pool based on CKPool - 0% Fee";
$current_pool_hashrate_1h = $pool_data['hashrate1hr'] ?? '0';
$current_pool_users = $pool_data['Users'] ?? '0';
$current_pool_workers = $pool_data['Workers'] ?? '0';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style> 
        :root { 
            ...
            --chart-color-1: #ff4444; 
            --chart-color-2: #ffaa00; 
            --chart-color-3: #ffcc00; 
            ...
            --muted-light: #475569;
            --gradient-start-light: #e0f2fe;
            --gradient-end-light: #ffffff;
            --accent-light: var(--accent-color-dark);
            --accent-hover-light: var(--accent-color);
            --accent-foreground-light: #ffffff;
            --text-shadow-light: none;
            --text-shadow-header-light: none;

            --background-dark: #000000;
            --foreground-dark: #f8f8f8;
            --card-background-dark: rgba(25, 0, 0, 0.7);
            --border-dark: rgba(255, 0, 0, 0.2);
            --input-dark: rgba(45, 0, 0, 0.6);
            --muted-dark: #ffaaaa;
            --gradient-start-dark: #300000;
            --gradient-end-dark: #000000;
            --accent-dark: var(--accent-color);
            --accent-hover-dark: var(--accent-color-dark);
            --accent-foreground-dark: #ffffff;
            --text-shadow-dark: 0 0 2px rgba(255, 0, 0, 0.1);
            --text-shadow-header-dark: 0 0 4px rgba(255, 0, 0, 0.2);
            
            --diff-up: #4ade80; --diff-down: #ff4444; 
        } 
        [data-theme='light'] { 
            --background: var(--background-light); --foreground: var(--foreground-light); 
            --card-background: var(--card-background-light); --border: var(--border-light); 
            --input: var(--input-light); --muted-foreground: var(--muted-light); 
            ...
            --text-shadow-header: var(--text-shadow-header-light);
        } 
        [data-theme='dark'] { 
            --background: var(--background-dark); --foreground: var(--foreground-dark); 
            --card-background: var(--card-background-dark); --border: var(--border-dark); 
            --input: var(--input-dark); --muted-foreground: var(--muted-dark); 
            ...
            --text-shadow-header: var(--text-shadow-header-dark);
        } 
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; } 
        ...
        .container { 
            ...
            padding: 2.5em; border-radius: 16px;
        ...
        h1, h2 { color: var(--foreground); border-bottom: 1px solid var(--border); padding-bottom: 0.5em; font-weight: 600; } 
        h1 { font-size: 1.75em; } 
        h2 { font-size: 1.25em; margin-top: 2.5em; text-align: center; } 
        h3 { margin-top: 0; font-size: 1.1em; color: var(--foreground); } 
        ...
        input[type="text"] { flex-grow: 1; padding: 12px 15px; font-size: 1em; font-family: var(--font-sans); border: 1px solid var(--border); border-radius: 8px 0 0 8px; background-color: var(--input); color: var(--foreground); transition: border-color 0.2s, box-shadow 0.2s; } 
        input[type="text"]:focus { outline: none; border-color: var(--accent); box-shadow: 0 0 0 4px color-mix(in srgb, var(--accent) 25%, transparent); } 
        input[type="submit"] { padding: 12px 24px; font-size: 1em; font-weight: 600; background-color: var(--accent); color: var(--accent-foreground); border: 1px solid var(--accent); border-left: 0; border-radius: 0 8px 8px 0; cursor: pointer; transition: background-color 0.2s; } 
        input[type="submit"]:hover { background-color: var(--accent-hover); border-color: var(--accent-hover); } 
        ...
        tr:last-child td { border-bottom: none; } td:not(.key) { font-family: var(--font-mono); } 
        /* ZMIANA: Szerokość 40% i font-size 1em */
        .key { font-weight: 600; color: var(--muted-foreground); width: 40%; font-family: var(--font-sans); font-size: 1em; } 
        a { color: var(--accent); text-decoration: none; font-weight: 600; } a:hover { text-decoration: underline; } 
        
        .footer { text-align: center; margin-top: 2.5em; font-size: 0.9em; color: var(--muted-foreground); } 
        ...
        .pool-header .metric-item { background-color: var(--card-background); padding: 0.5rem 1rem; border-radius: 6px; font-size: 0.9em; font-weight: 500; color: var(--foreground); display: flex; align-items: center; gap: 0.4rem; border: 1px solid var(--border); }
        .pool-header .metric-item strong { font-size: 1em; color: var(--foreground); font-family: var(--font-mono); }
        [data-theme='dark'] .pool-header .metric-item strong { text-shadow: none; }

        .theme-toggle { position: fixed; top: 1.25rem; right: 1.25rem; padding: 0; width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background-color: var(--card-background); border: 1px solid var(--border); box-shadow: 0 2px 5px rgba(0,0,0,0.05); overflow: hidden; transition: background-color 0.3s, border-color 0.3s, box-shadow 0.3s; z-index: 100; cursor: pointer; } 
        .theme-toggle:hover { background-color: var(--input); } .theme-toggle svg { position: absolute; transition: transform 0.3s ease-out, opacity 0.2s; color: var(--muted-foreground); } .theme-toggle .sun { transform: translateY(0); } .theme-toggle .moon { transform: translateY(150%); opacity: 0; } [data-theme='dark'] .theme-toggle .sun { transform: translateY(-150%); opacity: 0; } [data-theme='dark'] .theme-toggle .moon { transform: translateY(0); opacity: 1; } 
        ...
        .chart-controls button { font-family: var(--font-sans); font-size: 0.85em; font-weight: 600; padding: 0.5em 1em; border-radius: 6px; background-color: transparent; border: 1px solid var(--border); color: var(--muted-foreground); cursor: pointer; transition: background-color 0.2s, color 0.2s; } 
        .chart-controls button.active { background-color: var(--accent); color: var(--accent-foreground); border-color: var(--accent); }
        
        .connection-info-card { background-color: var(--input); border: 1px solid var(--border); border-radius: 8px; padding: 1.5rem; margin: 2.5em auto; font-size: 0.9em; max-width: 600px; } 
        .connection-info-card h2 { text-align: left; margin-top: 0; font-size: 1.1em; border-bottom: none; padding-bottom: 0; margin-bottom: 1em; } 
        .connection-info-card ul { list-style: none; padding-left: 0; text-align: left; }
        .connection-info-card code { background-color: color-mix(in srgb, var(--accent) 20%, transparent); color: var(--accent); padding: 0.2em 0.4em; border-radius: 4px; font-family: var(--font-mono); } .full-date { font-size: 0.85em; opacity: 0.7; margin-left: 0.5em; }
        ...
        .show-inactive-btn { font-size: 0.8em; background: none; border: none; cursor: pointer; text-decoration: underline; color: var(--diff-down); opacity: 0.8; } .show-inactive-btn:hover { opacity: 1; }
        ...
        .modal-backdrop { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.7); display: none; align-items: center; justify-content: center; z-index: 1000; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
        .modal-content { background-color: var(--card-background); padding: 2rem; border-radius: 12px; max-width: 90%; width: 800px; position: relative; border: 1px solid var(--border); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); }
        ...
        .modal-title { font-size: 1.25em; font-weight: 600; color: var(--foreground); }
        
        .diff-change { font-size: 0.9em; margin-left: 0.5em; font-weight: 600; }
        .diff-up { color: var(--diff-up); } .diff-down { color: var(--diff-down); }
...
<body>
    ...
    <div class="pool-header">
        <h1><?= htmlspecialchars($pool_title) ?></h1>
        <?php if ($pool_subtitle): ?>
            <p class="subtitle"><?= htmlspecialchars($pool_subtitle) ?></p>
        <?php endif; ?>
        <div class="pool-metrics">
            <div class="metric-item">Pool Hashrate (1h): <strong class="font-mono"><?= htmlspecialchars(format_hashrate($current_pool_hashrate_1h)) ?></strong></div>
            <div class="metric-item">Active Users: <strong class="font-mono"><?= htmlspecialchars($current_pool_users) ?></strong></div>
            <div class="metric-item">Total Workers: <strong class="font-mono"><?= htmlspecialchars($current_pool_workers) ?></strong></div>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="container">
        <?php if ($btc_address): ?>
            <div class="header-controls">
                <div>
                    <p style="margin-bottom: 1.5em;"><a href="<?= $script_path ?>">&larr; Back to Pool Overview</a></p>
                    <h1>User Statistics</h1>
                    <p style="word-wrap: break-word; font-family: var(--font-mono); color: var(--muted-foreground);"><?= $btc_address ?></p>
                </div>
            </div>
        <?php else: ?>
            <form action="<?= $script_path ?>" method="get"> <input type="text" name="btc_address" placeholder="Enter BTC address..." required><input type="submit" value="Search"> </form> 
            <div class="connection-info-card"> <h2>Connection Details</h2> <ul style="list-style: none; padding-left: 0;"> <li><strong>Stratum URL:</strong> <code>stratum+tcp://srv.88x.pl:3333</code></li> <li><strong>Username:</strong> <code>YOUR_BTC_ADDRESS</code></li> <li><strong>Password:</strong> <code>x</code></li> <li><strong>Pool Fee:</strong> <strong>0%</strong></li> </ul> </div> 
        <?php endif; ?>
        
        <?php if ($error): ?><p class="error"><?= $error ?></p><?php endif; ?>
        <?php if ($btc_address && !$user_summary && !$error): ?><p class="error">No data found for this BTC address.</p><?php endif; ?>
        
        <?php if (!$btc_address && ($network_difficulty || $network_hashrate)): ?>
            <h2 class="clickable-header" id="network-status-header" title="Click to see 30-day history chart">Network Status</h2>
            <table><tbody>
                <?php if ($network_hashrate): ?>
                <tr>
                    <td class="key">Network Hashrate</td>
                    <td class="font-mono">
                        <?= htmlspecialchars(format_hashrate($network_hashrate)) ?>
                        <?php if ($network_hashrate_change !== null):
                            $class = $network_hashrate_change >= 0 ? 'diff-up' : 'diff-down';
                            $sign = $network_hashrate_change >= 0 ? '+' : '';
                            echo ' <span class="diff-change ' . $class . '">(24h: ' . $sign . number_format($network_hashrate_change, 2) . '%)</span>';
                        endif; ?>
                    </td>
                </tr>
                <?php endif; ?>
                <?php if ($network_difficulty): ?>
                <tr>
                ...
                <tr>
                    ...
                    <td class="font-mono">
                        ...
                        if ($estimated_adjustment_date) {
                            echo '<br><span style="opacity: 0.7;">Est. Date: <strong>' . $estimated_adjustment_date . '</strong></span>';
                        }
                        ?>
                    </td>
                </tr>
        ...
        function render_table($data, $key_order, $friendly_names, $network_difficulty, $previous_difficulty, $analytics, $workers_data = null, $difficulty_prediction = null, $pool_time_to_block = null, $estimated_adjustment_date = null) {
            ...
            foreach ($key_order as $key) {
                if ($key === 'workers' && isset($data[$key]) && $data[$key] > 0) {
                    ...
                    if ($workers_data) {
                        $active_workers_html = '';
                        $inactive_workers_table = '<table><thead><tr><th class="key">Name</th><th>Hashrate (5 min)</th><th>Shares</th></tr></thead><tbody>';
                        foreach ($workers_data as $name => $stats) {
                            $hashrate5m = $stats['hashrate5m'] ?? '0';
                            if ((float)$hashrate5m > 0) {
                                $active_workers_html .= '<tr><td class="key">' . htmlspecialchars($name) . '</td><td>' . htmlspecialchars(format_hashrate($hashrate5m)) . '</td><td>' . htmlspecialchars(number_format($stats['shares'] ?? 0)) . '</td><td><button class="worker-chart-btn" data-worker="' . htmlspecialchars($name) . '">Show Chart</button></td></tr>';
                            } else {
                                $inactive_workers_table .= '<tr><td class="key">' . htmlspecialchars($name) . '</td><td>' . htmlspecialchars(format_hashrate($hashrate5m)) . '</td><td>' . htmlspecialchars(number_format($stats['shares'] ?? 0)) . '</td></tr>';
                                $has_inactive_workers = true;
                            }
                        }
                        ...
                        if ($has_inactive_workers) {
                           echo '<tr><td colspan="4" style="text-align: center; padding-top: 1em;"><button class="show-inactive-btn" id="show-inactive-btn">Show Inactive Workers</button></td></tr>';
                        }
                ...
                if ($key === 'time_to_block' && $pool_time_to_block) { echo '<tr><td class="key">' . ($friendly_names[$key] ?? 'Est. Time to Block') . '</td><td>' . htmlspecialchars(format_long_time($pool_time_to_block)) . '</td></tr>'; continue; }
                
                if (isset($data[$key])) {
                    $label = $friendly_names[$key] ?? ucfirst($key); $value = $data[$key];
                    echo '<tr><td class="key">' . htmlspecialchars($label) . '</td><td>';
                    if ($key === 'bestshare' && $workers_data !== null && $network_difficulty > 0) { // User's Best Share
                        ...
                        if ($difficulty_prediction) { echo '<div class="prediction-info">Next adjustment progress: <strong>' . ($difficulty_prediction['progress'] ?? 'N/A') . '%</strong>.<br>'; if (isset($difficulty_prediction['prediction'])) { $pred_val = $difficulty_prediction['prediction']; $pred_class = $pred_val >= 0 ? 'diff-up' : 'diff-down'; $pred_sign = $pred_val >= 0 ? '+' : ''; echo 'Estimated change: <strong class="' . $pred_class . '">' . $pred_sign . $pred_val . '%</strong>'; } if ($estimated_adjustment_date) { echo ' (Est. ' . $estimated_adjustment_date . ')'; } echo '</div>'; }
            ...
            echo '</tbody></table>';
            if (isset($has_inactive_workers) && $has_inactive_workers) {
                echo '<div id="inactive-workers-data" style="display:none;">' . $inactive_workers_table . '</div>';
            }
        }
        ?>
        
        <?php if ($user_summary): ?>
             <?php render_table($user_summary, ['hashrate1m', 'hashrate5m', 'hashrate1hr', 'hashrate1d', 'hashrate7d', 'shares', 'workers', 'lastshare', 'bestshare'], $friendly_names, $network_difficulty, $previous_network_difficulty, $analytics, $user_workers, $difficulty_prediction, null, $estimated_adjustment_date); ?>
        <?php endif; ?>
                    ...
                    const chartConfig = {
                        ...
                        options: {
                                ...
                                x: {
                                    ...
                                    time: {
                                        ...
                                        displayFormats: {
                                            ...
                                            day: 'MMM dd', week: 'MMM dd', month: 'MMM yyyy'
                                        }
                                    },
                                    grid: { color: borderColor },
                                    ticks: { color: mutedForegroundColor }
                                },
                                y: {
                                    beginAtZero: true,
                                    title: {
                            ...
                            plugins: {
                                ...
                                title: { display: true, text: title, font: { size: 16, weight: '600' }, color: foregroundColor },
                                tooltip: {
            ...
            const fetchAndRenderNetworkChart = async () => {
                ...
                try {
                    ...
                    modalChart = new Chart(modalChartCanvas, {
                        ...
                        data: {
                            datasets: [
                                { label: 'Network Hashrate (EH/s)', data: datasets.hashrate.labels.map((ts, index) => ({ x: ts, y: datasets.hashrate.data[index] / 1000000000 })), borderColor: color1, backgroundColor: color1 + '2A', yAxisID: 'yHashrate', tension: 0.1, fill: true, pointRadius: 0 },
                                { label: 'Network Difficulty', data: datasets.difficulty.labels.map((ts, index) => ({ x: ts, y: datasets.difficulty.data[index] })), borderColor: color2, backgroundColor: color2 + '2A', yAxisID: 'yDifficulty', tension: 0.1, fill: false, pointRadius: 0 }
                            ]
                        },
                        options: {
                            responsive: true, maintainAspectRatio: true,
                           ...
                           plugins: { legend: { labels: { color: mutedForegroundColor } }, title: { display: true, text: 'Network History (30 Days)', font: { size: 16, weight: '600' }, color: foregroundColor }, tooltip: { mode: 'index', intersect: false, backgroundColor: cardBackgroundColor, titleColor: foregroundColor, bodyColor: foregroundColor } }
                        }
                    });
                } catch (e) { console.error("Failed to fetch network chart data:", e); }
            };

            const closeModal = () => {
                modalBackdrop.style.display = 'none';
                if (modalChart) { modalChart.destroy(); modalChart = null; }
                document.getElementById('modal-chart-container').innerHTML = '<canvas id="modalChartCanvas"></canvas>';
            };
