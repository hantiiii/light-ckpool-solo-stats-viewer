...
try {
    ...
    if (!$btc_address && isset($pool_data['accepted']) && isset($pool_data['rejected'])) {
        ...
        if ($old_data) {
            ...
            $accepted_30d = $curr_acc; $rejected_30d = $curr_rej; $rejected_percent_30d = isset($pool_data['rejected_percent']) ? $pool_data['rejected_percent'] : 0;
        }
    }
...
if (!$btc_address && $accepted_30d !== null) {
    ...
    $pool_data['rejected_percent'] = $rejected_percent_30d;
...
if ($user_summary && $network_hashrate) { 
    ...
    $analytics = [ 'prob_month' => calculate_block_probability($user_hashrate_ghs, $network_hashrate, 30.44), 'prob_year' => calculate_block_probability($user_hashrate_ghs, $network_hashrate, 365.25), 'time_to_find' => $time_to_find_val ]; 
} 
...
$pool_title = "srv.88x.pl"; 
$pool_subtitle = "H.A.N.T.I. v7 Solaris & CKPool Node"; $current_pool_hashrate_1h = $pool_data['hashrate1hr'] ?? '0'; $current_pool_users = $pool_data['Users'] ?? '0'; $current_pool_workers = $pool_data['Workers'] ?? '0';
...
<head>
    ...
    <style>
        :root {
            ...
            --bg: #0f1115; --card-bg: #161b22; --border: #30363d;
            --text-main: #f0f6fc; --text-muted: #8b949e;
            --accent: #238636; --accent-glow: rgba(35, 134, 54, 0.4);
            --danger: #da3633; --warning: #d29922;
            --chart-1: #238636; --chart-2: #1f6feb; --chart-3: #a371f7;
        }
        [data-theme='light'] {
            --bg: #f6f8fa; --card-bg: #ffffff; --border: #d0d7de;
            --text-main: #24292f; --text-muted: #57606a;
            --accent: #1a7f37; --accent-glow: rgba(26, 127, 55, 0.2);
        }
...
<header>
    <div class="brand">
        <h1>srv.88x.pl <span>NODE</span></h1>
        <div class="subtitle">Solo Mining Intelligence • H.A.N.T.I. v7 Solaris</div>
    </div>
...
<div class="dashboard-grid">
    ...
    <div class="card kpi-card">
        <div>
            <div class="kpi-title">Network Hashrate</div>
            <div class="kpi-value"><?= htmlspecialchars(format_hashrate($network_hashrate)) ?></div>
            <?php if ($network_hashrate_change !== null): 
                $class = $network_hashrate_change >= 0 ? 'text-green' : 'text-red'; 
                $sign = $network_hashrate_change >= 0 ? '+' : ''; 
                echo '<div class="kpi-sub '.$class.'">24h Change: '.$sign.number_format($network_hashrate_change, 2).'%</div>'; 
            endif; ?>
        </div>
        <div class="kpi-sub clickable" id="network-status-header">View History Chart</div>
    </div>

    <div class="card kpi-card hanti-box">
        <div>
            <div class="hanti-header">
                <div class="kpi-title">Difficulty Prediction</div>
                <div class="hanti-badge">HANTI v7</div>
            </div>
            <?php if ($difficulty_prediction && isset($difficulty_prediction['prediction'])): 
                $pred = $difficulty_prediction['prediction'];
                $p_class = $pred >= 0 ? 'text-green' : 'text-red';
                $p_sign = $pred >= 0 ? '+' : '';
            ?>
            <div class="kpi-value <?= $p_class ?>"><?= $p_sign . number_format($pred, 2) ?>%</div>
            <div class="kpi-sub">
                Progress: <?= $difficulty_prediction['progress'] ?>% <br>
                Est: <?= $estimated_adjustment_date ?>
            </div>
            <?php else: ?>
            <div class="kpi-value">Calc...</div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="card kpi-card">
        <div>
            <div class="kpi-title">Current Difficulty</div>
            <div class="kpi-value" style="font-size: 1.4rem;"><?= htmlspecialchars(format_number_auto($network_difficulty)) ?></div>
            <?php if ($previous_network_difficulty): 
                $diff_chg = (($network_difficulty - $previous_network_difficulty)/$previous_network_difficulty)*100;
                $d_class = $diff_chg >= 0 ? 'text-green' : 'text-red';
                echo '<div class="kpi-sub '.$d_class.'">Prev: '.($diff_chg>=0?'+':'').number_format($diff_chg, 2).'%</div>';
            endif; ?>
        </div>
    </div>
    ...
    <div class="card kpi-card">
        <div>
            <div class="kpi-title">Block Reward</div>
            <div class="kpi-value text-green">$<?= $last_block_reward_usd ? number_format($last_block_reward_usd, 0, '.', ' ') : '---' ?></div>
            <div class="kpi-sub"><?= number_format($last_block_reward_btc, 6) ?> BTC</div>
            <div class="kpi-sub" style="font-size:0.75rem; opacity:0.6;">Block #<?= number_format($last_fetched_block_height) ?></div>
        </div>
    </div>
    ...
    <div class="card full-width">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
            <h3><?= $btc_address ? 'Worker Performance' : 'Pool Hashrate History' ?></h3>
            <div class="chart-controls" id="chart-controls">
                <button class="chart-btn active" data-range="1">24H</button>
    ...
    <div class="card full-width">
        <?php if ($btc_address): ?> 
            <div style="display:flex; justify-content:space-between;">
                <h3>User: <span class="font-mono"><?= substr($btc_address, 0, 8) ?>...</span></h3>
                <a href="<?= htmlspecialchars($script_path) ?>" style="color:var(--accent);">Back to Pool</a>
            </div>
            <?php if ($user_summary): ?> 
                <?php render_table($user_summary, ['hashrate1m', 'hashrate5m', 'hashrate1hr', 'hashrate1d', 'hashrate7d', 'shares', 'workers', 'lastshare', 'bestshare'], $friendly_names, $network_difficulty, $previous_network_difficulty, $analytics, $user_workers, null, null, null); ?> 
            <?php else: ?><p class="error">No data found.</p><?php endif; ?>
        <?php else: ?> 
            <h3>Check Your Stats</h3>
            <form action="<?= htmlspecialchars($script_path) ?>" method="get" style="display:flex; margin-top:1rem;"> 
                <input type="text" name="btc_address" placeholder="Enter BTC address..." required>
                <input type="submit" value="Search"> 
            </form>
            <div style="margin-top:2rem;">
                <h3 style="margin-bottom:1rem;">Pool Statistics</h3>
                <?php if ($pool_data): ?>
...
<div id="modal-backdrop" class="modal-backdrop"> 
    <div class="modal-content"> 
        ...
        <h2 id="modal-title" style="margin-bottom:1rem;">Chart</h2> 
        <div style="height:400px; width:100%;"><canvas id="modalChartCanvas"></canvas></div> 
    </div> 
</div>

<?php 
// Render Table Helper Definition
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function loadChart(range, worker = null, canvas = mainCanvas, isModal = false) {
            ...
            for(const k in data) {
                if(data[k].data) {
                    datasets.push({
                        label: k,
                        data: data[k].labels.map((t, i) => ({x: t*1000, y: data[k].data[i]})),
            ...
            const config = {
                ...
                options: {
                    ...
                    scales: {
                        x: { type: 'time', time: { unit: range > 7 ? 'day' : 'hour' }, grid: { color: '#30363d' } },
            ...
            if(isModal) {
                ...
                window.modalChart = new Chart(canvas, config);
            } else {
        ...
        document.getElementById('network-status-header')?.addEventListener('click', async () => {
            ...
            window.modalChart = new Chart(ctx, {
                ...
                data: {
                    datasets: [
                        { label: 'Hashrate', data: d.hashrate.labels.map((t,i)=>({x:t, y:d.hashrate.data[i]})), borderColor: '#238636', yAxisID: 'y' },
                        { label: 'Difficulty', data: d.difficulty.labels.map((t,i)=>({x:t, y:d.difficulty.data[i]})), borderColor: '#a371f7', yAxisID: 'y1' }
                    ]
                },
                options: {
