...
try {
    ...
    // BEZPIECZNY TRYB (NO WAL)
    $pdo->exec("PRAGMA journal_mode = DELETE");
    $pdo->exec("PRAGMA synchronous = NORMAL");
    
    // FixSchema: Jeśli tabela jest stara, usuwamy ją (dane są ulotne)
    $checkCols = $pdo->query("PRAGMA table_info(prediction_data)")->fetchAll(PDO::FETCH_COLUMN, 1);
    if (!empty($checkCols) && !in_array('timestamp', $checkCols)) { $pdo->exec("DROP TABLE prediction_data"); }
...
echo "--- H.A.N.T.I. Prediction v74 (Safe Mode) ---\n";
...
$stmt->execute([$timeNow, round($progressPercent, 2), round($changePercent, 2), (int)$estimatedDate, "H.A.N.T.I. v7 Active"]);

// 6. Historia (Zapisz jeśli minęła godzina od ostatniego wpisu lub pusto)
